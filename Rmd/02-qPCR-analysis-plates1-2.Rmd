...
which takes into account the sample volume (1 L) and the elution volume (100 ul). 

In that case, the number of copies per qPCR reaction (Quantity) is equivalent to 20 mL or 0.02 L of water sample collected in the field.

The adjusted copy number = qPCR Quantity/0.02 L = DNA copies per L.
...
data.qpcr <- test_data %>%
  ...
  mutate(copies_per_L = Quantity/0.02) %>%
  mutate(mean_copies_per_L = `Quantity Mean`/0.02)
...
ggsave("pdf_outputs/qPCR_detections_tile.pdf", width = 7, height = 3)
```
